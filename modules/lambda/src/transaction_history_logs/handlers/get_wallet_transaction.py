...
def lambda_handler(event: Dict[str, Any], context) -> Dict[str, Any]:
    ...
    # Validate ret_ref_id - return 400 Bad Request
    if not request_data.ret_ref_id or request_data.ret_ref_id.strip() == "":
        raise BadRequestException(
            custom_error_details="ret_ref_id is required"
        )
    if len(request_data.ret_ref_id) > 500:
        raise BadRequestException(
            custom_error_details="ret_ref_id must not exceed 500 characters"
        )
