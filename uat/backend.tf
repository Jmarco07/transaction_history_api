...
terraform {
  backend "s3" {
    bucket         = "ppay-redshift-api-tfstate-bucket-uat"
    key            = "uat/palawanpay-redshift-api/terraform.tfstate"
    dynamodb_table = "ppay-redshift-api-tfstate-lock-table-uat"
...
module "api_gateway" {
  ...
  apigw_lambda_list = [
    ...
    },
    {
      ...
      route_key       = "POST /v1/api/wallet-transaction-history",
      integration_uri = module.lambda_functions.lambda_arns["wallet_transaction"]
      requires_auth   = true
    },
    {
      function_name   = "remittance_transaction",
      route_key       = "POST /v1/api/remittance-transaction-history",
      integration_uri = module.lambda_functions.lambda_arns["remittance_transaction"]
      requires_auth   = true
    },
    {
      function_name   = "bulk_pepp_disbursement",
      route_key       = "POST /v1/api/bulk-pepp-disbursement-history",
      integration_uri = module.lambda_functions.lambda_arns["bulk_pepp_disbursement"]
      requires_auth   = true
    },
    {
...
module "lambda_functions" {
  ...
  lambda_function_list = [
    ...
    },
    {
      stack_name    = "transaction_history_logs",
      function_name = "wallet_transaction",
      description   = "Redshift API Wallet Transaction History function",
      role          = "${local.common.project_name}-wallet_transaction-lambda-role-${local.common.environment}",
      handler       = "handlers.get_wallet_transaction.lambda_handler",
      runtime       = "python3.11",
      memory_size   = 256,
      timeout       = 30,
      layers        = ["redshift-api-layer-v1"],
    ...
    },
    {
      stack_name    = "transaction_history_logs",
      function_name = "bulk_pepp_disbursement",
      description   = "Redshift API Bulk PEPP Disbursement Transaction History function",
      role          = "${local.common.project_name}-bulk_pepp_disbursement-lambda-role-${local.common.environment}",
      handler       = "handlers.get_bulk_pepp_disbursement.lambda_handler",
      runtime       = "python3.11",
      memory_size   = 1536,
      timeout       = 120,
      layers        = ["redshift-api-layer-v1"],
